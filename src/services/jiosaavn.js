...
import { filterSongsByLanguage } from '../utils/languageFilter.js';
...
// Applies to every home/discovery section (NOT to raw searchSongs — Search.jsx
// needs to control language-bypass intent itself). Chapri blocklist is always
// active inside filterSongsByLanguage; the language check only kicks in if
// the user has the language filter setting on (default: on).
function curateResults(songs) {
  return filterSongsByLanguage(songs);
}
...
export async function getRecommendations(likedSongObjects = [], recentlyPlayedSongs = [], skippedSongs = {}) {
  try {
    ...
    return curateResults(filtered).slice(0, 15);
  } catch {
    return [];
  }
}
...
export async function getDailyMix(likedSongObjects = []) {
  ...
  // v2: bumped after wiring up language/blocklist filtering — old v1 cache
  // entries (possibly unfiltered) are intentionally never read again.
  const cacheKey = `geet_daily_mix_v2_${today}`;

  // One-time cleanup of stale pre-v2 daily mix cache entries, so they don't
  // sit around forever in localStorage.
  try {
    Object.keys(localStorage)
      .filter(k => k.startsWith('geet_daily_mix_') && !k.startsWith('geet_daily_mix_v2_'))
  ...
  try {
    ...
    // Shuffle for variety
    const shuffled = curateResults(mix).sort(() => Math.random() - 0.5).slice(0, 20);
...
export async function getHiddenGems(likedSongObjects = [], recentlyPlayedSongs = []) {
  ...
  try {
    ...
    const seen = new Set();
    const deduped = results.filter(song => {
      if (!song || seen.has(song.id) || knownIds.has(song.id)) return false;
      seen.add(song.id);
      return true;
    });
    return curateResults(deduped).slice(0, 15);
  } catch {
    return [];
  }
}
