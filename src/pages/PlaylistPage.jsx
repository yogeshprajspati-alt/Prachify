...
export default function PlaylistPage() {
  ...
  const removeSongFromSharedPlaylist = usePlayerStore(s => s.removeSongFromSharedPlaylist);
  const reorderSharedPlaylistSongs = usePlayerStore(s => s.reorderSharedPlaylistSongs);
  ...
  const isShared = id === 'shared-family';
  ...
  const handleDrop = () => {
    ...
    if (isShared) reorderSharedPlaylistSongs(songs); else reorderPlaylistSongs(id, songs);
  ...
  return (
    <div className="page" style={{ background: '#121212' }}>
      ...
      {/* ── Action pill row (custom + shared playlists) ──────────────── */}
      {(isCustom || isShared) && (
        <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 16px 4px', justifyContent: 'flex-end' }}>
        {playlist.songs.length > 5 && (
          <button
            ...
            style={{
              background: searchOpen ? 'rgba(29,185,84,0.15)' : 'none',
              border: 'none', borderRadius: '50%', cursor: 'pointer', padding: 6,
              marginRight: 'auto',
            }}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={searchOpen ? '#1DB954' : '#b3b3b3'} strokeWidth="2" strokeLinecap="round">
              <circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/>
            </svg>
          </button>
        )}
            ...
            >
              ...
              {!reorderMode && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 2, flexShrink: 0 }}>
                  ...
                  {(isCustom || isShared) && (
                    <button
                      onClick={e => { e.stopPropagation(); isShared ? removeSongFromSharedPlaylist(song.id) : removeSongFromPlaylist(id, song.id); }}
