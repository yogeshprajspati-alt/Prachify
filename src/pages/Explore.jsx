...
import { filterSongsByLanguage } from '../utils/languageFilter';
...
// Note: language + chapri-blocklist filtering now goes through the shared
// filterSongsByLanguage() util (respects the Settings toggle), applied
// directly at each call site below.

export default function Explore() {
  const navigate = useNavigate();
  const { currentSong, isPlaying, playSong } = usePlayer();
  const activeProfileId = useProfileStore(s => s.activeProfileId);
  const isPrachi = activeProfileId === 'prachi';
  ...
  // (Prachi-only section — other profiles never fetch or see this.)
  useEffect(() => {
    if (!isPrachi) return;
    ...
    Promise.all(
      ...
      const found = filterSongsByLanguage(results.filter(Boolean), { languageCheck: false });
      setCached(cacheKey, found);
      setDedicatedSongs(found);
      setDedicatedLoading(false);
    });
  }, [isPrachi]); // eslint-disable-line
  ...
  useEffect(() => {
    exploreCats.forEach(cat => {
      ...
      searchSongs(query, 20).then(songs => {
        ...
        const filtered = filterSongsByLanguage(songs).slice(0, 12);
  ...
  return (
    <div className="page" style={{ background: '#121212', paddingBottom: 100 }}>
      <div style={{ padding: 'max(env(safe-area-inset-top), 24px) 16px 24px' }}>
      ...
      {/* ── Dedicated to Prachi (Prachi-only) ── */}
      {isPrachi && (
