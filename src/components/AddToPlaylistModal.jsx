...
export default function AddToPlaylistModal({ show, song, onClose }) {
  ...
  const addSongToSharedPlaylist = usePlayerStore(s => s.addSongToSharedPlaylist);
      ...
      <div style={{ position: 'relative', width: '100%', maxWidth: 430, background: '#282828', borderRadius: '16px 16px 0 0', padding: '20px 0', maxHeight: '70vh', overflowY: 'auto' }}>
        ...
        {activeProfileId !== 'guest' && (
          <button onClick={handleShared}
            ...
            <img src={sharedPlaylist.cover} alt="" style={{ width: 44, height: 44, borderRadius: 4, objectFit: 'cover' }} onError={e => e.target.style.background='#333'} />
            <div style={{ textAlign: 'left' }}>
              <div style={{ fontSize: 14, fontWeight: 600, color: '#fff' }}>{sharedPlaylist.title} 👨‍👩‍👧</div>
              <div style={{ fontSize: 12, color: '#b3b3b3' }}>{sharedPlaylist.songs.length} songs</div>
            </div>
          </button>
        )}
        {/* Existing playlists */}
