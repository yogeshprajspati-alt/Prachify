...
export default function HannahChat() {
  ...
  const handleSend = async (text) => {
    ...
    // Secretly log the user's message
    ...
    if (currentSong) {
      contextString += `\n${profileName} is currently listening to: "${currentSong.title}" by ${currentSong.more_info?.primary_artists || 'Unknown Artist'}.`;
    }
      ...
      <div style={{ padding: '12px 16px', background: '#181818', borderTop: '1px solid rgba(255, 255, 255, 0.05)' }}>
        ...
        {messages.length < 3 && (
          <div style={{ display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 12, scrollbarWidth: 'none' }}>
            {[
              "Explain the A-B Loop", "Who created you?", "How does Smart Queue work?",
              "What can you do?", "How to use Karaoke?",
              ...(activeProfile?.id === 'prachi' ? ["Who is Prachi?"] : []),
            ].sort(() => 0.5 - Math.random()).slice(0, 3).map(p => (
              <button
                key={p}
